...
async fn get_unknown<'a>() -> Result<ResponseUnknown, Err> {
    let unknown = echo.get_unknown("https://httpbin.org/get").await?;
    Ok(unknown)
}

async fn get<'a>() -> Result<Response<Vec<Post>>, Err> {
    let posts = echo
...
async fn post<'a>() -> Result<Response<Post>, Err> {
    let new_post = Post {
        user_id: 1,
        id: 1,
...
async fn put<'a>() -> Result<Response<Post>, Err> {
...
async fn delete<'a>() -> Result<ResponseUnknown, Err> {
    let deleted = echo
